...
if [[ "$tool_value" == "vcs" ]]; then
  ...
  # ADDED: Optional full hierarchy visibility in the FSDB waveform.
  #
  # Enable this with:
  #   VCS_FULL_DEBUG=1 ./run_sim_full_debug.sh ... --waves fsdb
  #
  # This deliberately does not enable --gui. It temporarily changes only the
  # vcs_waves build option from -debug_access to -debug_access+all, and the
  # original dvsim configuration is restored when this wrapper exits.
  vcs_dvsim_cfg="$PWD/hw/dv/tools/dvsim/vcs.hjson"
  vcs_dvsim_cfg_backup=""

  restore_vcs_dvsim_cfg() {
    if [[ -n "${vcs_dvsim_cfg_backup:-}" &&
          -n "${vcs_dvsim_cfg:-}" &&
          -f "$vcs_dvsim_cfg_backup" ]]; then
      cp "$vcs_dvsim_cfg_backup" "$vcs_dvsim_cfg"
      rm -f "$vcs_dvsim_cfg_backup"
      vcs_dvsim_cfg_backup=""
      echo "Restored original VCS dvsim configuration." >&2
    fi
  }

  if [[ "${VCS_FULL_DEBUG:-0}" == "1" ]]; then
    if [[ ! -f "$vcs_dvsim_cfg" ]]; then
      echo "ERROR: VCS dvsim configuration not found: $vcs_dvsim_cfg" >&2
      exit 1
    fi

    vcs_dvsim_cfg_backup="$(mktemp)"
    cp "$vcs_dvsim_cfg" "$vcs_dvsim_cfg_backup"
    trap restore_vcs_dvsim_cfg EXIT INT TERM

    python3 - "$vcs_dvsim_cfg" <<'PY'
...
start = text.find("name: vcs_waves")
if start < 0:
    raise SystemExit(f"ERROR: vcs_waves entry not found in {path}")

end = text.find("\n    }", start)
if end < 0:
    raise SystemExit(f"ERROR: end of vcs_waves entry not found in {path}")

block = text[start:end]
normal_option = '"-debug_access"'
full_option = '"-debug_access+all"'

if normal_option in block:
    block = block.replace(normal_option, full_option, 1)
    path.write_text(text[:start] + block + text[end:])
    print(f"Enabled full VCS waveform visibility in {path}", file=sys.stderr)
elif full_option in block:
    print(f"Full VCS waveform visibility is already enabled in {path}", file=sys.stderr)
else:
    raise SystemExit(
        f"ERROR: expected -debug_access option not found in vcs_waves entry of {path}"
    )
PY
  fi
...
PY
  ...
  # ADDED: Always restore the VCS config, CSRNG coverage source and AES model
  # source after the VCS run, including after Ctrl-C or a failure.
  restore_vcs_sources() {
    if [[ -n "${vcs_cov_backup:-}" &&
          -n "${vcs_cov_file:-}" &&
